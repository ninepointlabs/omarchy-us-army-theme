# Insignia sources (Wikimedia Commons)

- `3id.svg`: File:3rd Infantry Division SSI (1918-2015).svg — Public domain — https://commons.wikimedia.org/wiki/File:3rd_Infantry_Division_SSI_(1918-2015).svg
- `2id.svg`: File:2nd Infantry Division SSI (full color).svg — Public domain — https://commons.wikimedia.org/wiki/File:2nd_Infantry_Division_SSI_(full_color).svg
- `24id.svg`: File:24 Infantry Division SSI.svg — Public domain — https://commons.wikimedia.org/wiki/File:24_Infantry_Division_SSI.svg
- `1cav.svg`: File:1st Cavalry Division SSI (full color).svg — Public domain — https://commons.wikimedia.org/wiki/File:1st_Cavalry_Division_SSI_(full_color).svg
- `35id.svg`: File:35th Infantry Division SSI.svg — Public domain — https://commons.wikimedia.org/wiki/File:35th_Infantry_Division_SSI.svg
- `cib.svg`: File:Combat Infantry Badge.svg — Public domain — https://commons.wikimedia.org/wiki/File:Combat_Infantry_Badge.svg
- `purpleheart.png`: File:Purple Heart Medal.png — Public domain — https://commons.wikimedia.org/wiki/File:Purple_Heart_Medal.png
- `arcom.png`: File:ArmyCommMed.png — Public domain — https://commons.wikimedia.org/wiki/File:ArmyCommMed.png
- `vdevice.svg`: File:"V" device, brass.svg — Public domain — https://commons.wikimedia.org/wiki/File:%22V%22_device,_brass.svg
- `sgt.svg`: File:Army-USA-OR-05-2015.svg (Sergeant, E-5) — Public domain — https://commons.wikimedia.org/wiki/File:Army-USA-OR-05-2015.svg
